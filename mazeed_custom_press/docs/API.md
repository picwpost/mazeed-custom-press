...
Fetches the first ready standby site for a Release Group (same logic as `get_standby_site_for_release_group`) and immediately prefills its Setup Wizard via a synchronous HTTP call to the live Frappe instance.
...
|---|---|---|---|
| `release_group` | string | yes | Release Group name or title |
| `system_settings` | dict | yes | System-level wizard settings |
| `user_settings` | dict | yes | User account settings |

`system_settings` shape:

| Key | Description |
|---|---|
| `country` | Country name (e.g. `"Saudi Arabia"`) |
| `time_zone` | IANA timezone (e.g. `"Asia/Riyadh"`) |
| `language` | Language code — typically `"en"` |
...
**Notes**
- The call blocks until the remote Setup Wizard initialization completes.
- If the site's `setup_wizard_complete` is already `1`, the prefill step is silently skipped.
